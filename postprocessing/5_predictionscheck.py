...
#name = 'reddit_depression_suicidewatch_MLP_CountVectorize'
#name = 'test_CountVectorize'
#name = 'test_MLP_CountVectorize'
#name = 'suicide_notes_MLP_CountVectorize'
...
with open('words/true_positives/true_positives_'+name+'.csv', 'w') as f:
    for key in lst:
        val = lst[key]*10000
        val = np.ceil(val)
        #rint(val)
...
with open('words/false_positives/false_positives_'+name+'.csv', 'w') as f:
    for key in lst:
        val = lst[key]*10000
        val = np.ceil(val)
        if val < 1:
...
with open('words/false_negatives/false_negatives_'+name+'.csv', 'w') as f:
    for key in lst:
        val = lst[key]*10000
        val = np.ceil(val)
        if val < 1:
